...
function getCollection (cb) {
  ...
  MongoClient.connect(urljoin(config.get('MONGO_URL'),config.get('MONGO_DATABASE')), function (err, db) {
    if (err) {
      console.log(err);
      return cb(err);
    }
...
// [START findby]
function findBy (emailaddress, limit, token, cb) {
  token = token ? parseInt(token, 10) : 0;
  if (isNaN(token)) {
    return cb(new Error('invalid token'));
  }
  getCollection(function (err, collection) {
    ...
    collection.find({email: emailaddress})
      .skip(token)
      .limit(limit)
      .toArray(function (err, results) {
        if (err) {
